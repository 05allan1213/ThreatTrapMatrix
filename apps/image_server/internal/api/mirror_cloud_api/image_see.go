...
type ImageSeeResponse struct {
	ImageID   string `json:"imageID"`   // 镜像ID（Config哈希值，非Docker真实镜像ID）
	ImageName string `json:"imageName"` // 镜像名称
	ImageTag  string `json:"imageTag"`  // 镜像标签
	ImagePath string `json:"imagePath"` // 镜像临时上传存储路径
}

const (
	maxFileSize  = 2 << 30                // 镜像文件大小上限：2GB
	tempImageDir = "uploads/images_temp/" // 镜像文件临时存储目录路径
)

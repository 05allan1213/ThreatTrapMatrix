...
// Description: 实现节点侧批量部署MQ消息消费逻辑，拆分核心部署逻辑为独立函数，包含任务入库、异步执行IP配置/端口转发、部署状态上报及任务状态更新等功能
...
func DeployTask(req models.BatchDeployRequest, taskID string) {
	...
	// 总部署IP数量，用于计算部署进度（1-100的小数）
	var allCount = int64(len(req.IPList))
	// 互斥锁：保证部署状态上报、进度计算时的数据一致性
	...
	for _, ip := range req.IPList {
		...
		go func(info models.DeployIp, wait *sync.WaitGroup) {
			...
			// 生成唯一网络接口名称（前缀hy_+6位唯一随机字符串，避免冲突）
			...
			// 加锁保证进度计算和状态上报的原子性，避免并发数据错乱
			mutex.Lock()
			// 原子更新已完成部署的IP数量，确保计数准确
